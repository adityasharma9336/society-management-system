...
import PaymentUI from '../components/billing/PaymentUI';
...
export default function ResidentBilling() {
    ...
    const handlePaySuccess = async (id, paymentData) => {
        try {
            await payBill(id, paymentData);
                                    ...
                                    {loading ? (
                                        ...
                                        bills.map(bill => (
                                            <tr key={bill._id} className="border-b border-slate-100 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800/50">
                                                ...
                                                <td className="py-3 px-4">
                                                    {bill.status !== 'paid' && (
                                                        <button onClick={() => setSelectedBill(bill)} className="text-sm font-bold text-primary hover:underline">Pay Now</button>
                                                    )}
                                                </td>
                                            </tr>
                                        ))
